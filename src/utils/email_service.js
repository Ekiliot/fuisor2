...
export async function sendOTPEmail(email, otpCode) {
  try {
    ...
    const template = await readEmailTemplate('otp_email_template.html');
    ...
    if (resendApiKey) {
      ...
      console.log(`[EMAIL] OTP sent successfully to ${email} via Resend:`, result.id);
      return;
    }
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`[EMAIL] Resend API key not configured. OTP for ${email}: ${otpCode}`);
